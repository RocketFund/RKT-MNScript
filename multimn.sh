...
# Author: Rocketfundcoin 
# Source: https://github.com/RocketFund/RKT-MNScript
...
function cmd_bootstrap() {
	...
	#For copied bootstrap data while new node installation // Ex. : multimn install rocketfundcoin  --bootstrap
    cp -r $(get_folder $1)blocks $(get_folder $2)
    cp -r $(get_folder $1)chainstate $(get_folder $2)
    cp -r $(get_folder $1)sporks $(get_folder $2)
    cp -r $(get_folder $1)zerocoin $(get_folder $2)	
...
function cmd_help() {
	echo -e "Options:\
			...
			\n**NOTE 2**: Check ${CYAN}https://github.com/RocketFund/RKT-MNScript/wiki/Commands${NC} for extended info and usage examples of each command.\
			\n**NOTE 3**: Check ${CYAN}https://github.com/lRocketFund/RKT-MNScript/wiki/FAQs${NC} for technical questions and troubleshooting."
}
function cmd_update() {
	curl -sL https://raw.githubusercontent.com/RocketFund/RKT-MNScript/master/multimn_install.sh 3>&3 | bash
	exit
}
